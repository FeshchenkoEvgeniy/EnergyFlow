...
function createMarkupForFavoriteSubtypeExercises(results: ISubtypeExercise[]): string {
      return results.map(({ bodyPart, burnedCalories, name, target, time, _id }: ISubtypeExercise) => {
        return `
        <li class="subtype-exercises__item js-subtype-exercises__item" data-id="${_id}">
  <div class="subtype-exercises__layout">
    <p class="subtype-exercises__workout">WORKOUT</p>
...
function createMarkupForFavoriteExercisesModal({ bodyPart, burnedCalories, description, equipment, gifUrl, name, popularity, rating, target, time, _id}: ISubtypeExercise): string {
  const star = Math.round(rating)
  const arrStr:string[] = []
  for (let i = 0; i < star; i++) {
    arrStr.push(`<svg width="14" height="13" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export { createMarkupForExercises, createMarkupForExercisesPagination, createMarkupForSubtypeExercises, createMarkupForSubtypeExercisesModal, createMarkupForFavoriteSubtypeExercises, createMarkupForFavoriteExercisesModal}
